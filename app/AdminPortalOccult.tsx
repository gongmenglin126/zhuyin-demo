...
type AdminDetail="pair2004"|"lin"|"reswap"|"third"|"sync"|null;
...
function LegacyVerify({onBack,onVerified}:{onBack:()=>void;onVerified:()=>void}){
 ...
 const giveItem=(target:Child|"center")=>{
  ...
  if(id==="box"){
   if(target==="center"){const next={...items,box:"center" as const};setItems(next);setMoods({lin:"neutral",shen:"neutral"});setDragItem(null);completeTable(next);return;}
   const other:Child=target==="lin"?"shen":"lin";setMoods(m=>({...m,[target]:"neutral",[other]:"smile"}));setDragItem(null);window.setTimeout(()=>setMoods(m=>({...m,[other]:"neutral"})),850);return;
  }
  if(target==="center"){setDragItem(null);return;}
  if(itemAnswer[id]===target){const next={...items,[id]:target};setItems(next);setDragItem(null);completeTable(next);return;}
 ...
 return <main style={s.verifyPage}><section style={s.verifyShell}>
  ...
  {stage===2&&<div style={v.tableScene}><div style={v.tableSeats}><TableSeat place="4栋东侧" mood={moods.lin} side="left" onDrop={()=>giveItem("lin")} items={ordinaryItems.filter(id=>items[id]==="lin")}/><div style={v.centerTable} onDragOver={e=>e.preventDefault()} onDrop={()=>giveItem("center")}><div style={v.tableTop}>{(["plum","marble","milk","clip","box"] as Item[]).filter(id=>!items[id]).map(id=><ObjectToken key={id} id={id} draggable onDragStart={()=>setDragItem(id)}/>)}{items.box==="center"&&<ObjectToken id="box" opened/>}</div></div><TableSeat place="青梧旧楼" mood={moods.shen} side="right" onDrop={()=>giveItem("shen")} items={ordinaryItems.filter(id=>items[id]==="shen")}/></div></div>}
...
function ObjectToken({id,draggable,onDragStart,opened}:{id:string;draggable?:boolean;onDragStart?:()=>void;opened?:boolean}){const labels:Record<string,string>={plum:"话梅糖",marble:"蓝玻璃弹珠",milk:"奶糖",clip:"红色发卡",box:"红铁皮盒"};return <div draggable={draggable} onDragStart={e=>{e.dataTransfer.effectAllowed="move";onDragStart?.()}} style={{...v.object,cursor:draggable?"grab":"default"}}><i style={{...v.objectIcon,position:"relative",...(id==="plum"?v.plum:id==="marble"?v.marble:id==="milk"?v.milk:id==="clip"?v.clip:v.box)}}>{id==="box"&&opened&&<span style={{position:"absolute",left:1,right:1,top:-7,height:7,border:"1px solid #704236",borderBottom:0,background:"#7d2b23",transform:"rotate(-7deg)",transformOrigin:"left bottom",boxShadow:"0 -2px 8px #0008"}}/>}</i><small>{labels[id]}</small></div>}
...
function AdminDesk({onWechatIncoming,onCopyMaterial,hasMaterial}:{onWechatIncoming?:()=>void;onCopyMaterial?:(m:SharedMaterial)=>void;hasMaterial?:(id:string)=>boolean}){
 ...
 const result=useMemo<ResultKind>(()=>{if(!searched)return null;const t=q.trim();if(/候鸟第七年|沈妍|0712-4471/.test(t))return "shen";if(/迟迟|梁茵|0419-2286/.test(t))return "liang";if(/林楠|LN-2004-0718/.test(t))return "lin";if(/19-07|候舍/.test(t))return "third";return null},[searched,q]);
 useEffect(()=>{adminDeskSession.tab=tab;adminDeskSession.q=q;adminDeskSession.searched=searched;adminDeskSession.detail=detail},[tab,q,searched,detail]);
 ...
 const doSearch=(e?:FormEvent)=>{e?.preventDefault();setSearched(true);setDetail(null);if(/候鸟第七年|沈妍|0712-4471/.test(q.trim()))fireShenBeat()};
 const openKnown=(name:string)=>{setQ(name);setSearched(true);setDetail(null);setTab("users");if(/候鸟第七年|沈妍/.test(name))fireShenBeat()};
 const openDetail=(next:AdminDetail)=>{setDetail(next);setTab("users")};
...
function ShenRecord({openDetail,onCopyMaterial,hasMaterial}:{openDetail:(x:AdminDetail)=>void;onCopyMaterial?:(m:SharedMaterial)=>void;hasMaterial?:(id:string)=>boolean}){return <article style={s.userRecord}>
 ...
 <section style={s.adminPanel}><h4>关联信息</h4><Record date="旧案" title="2004-07-17" meta="年龄 9 · 失踪 13 天" text="关联旧案：LN-2004-0718；关联对象：林楠。"/><DetailLink onClick={()=>openDetail("pair2004")}>查看旧案 LN-2004-0718</DetailLink><Record date="首次录入" title="2021-06-14" meta="自动索引匹配" text="论坛实名映射完成；进入长期观察。"/></section>
