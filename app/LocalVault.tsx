...
export default function LocalVault({unlocked,onUnlock,openLink}:{unlocked:boolean;onUnlock:()=>void;openLink:()=>void}){
  ...
  const submit=(e:FormEvent)=>{
    e.preventDefault();
    if(normalize(pwd)==="红铁皮盒"){
      setError("");
      onUnlock();
      return;
    }
    setError(pwd.trim()?"口令不正确":"请输入口令");
  };
...
const styles:{[key:string]:React.CSSProperties}={
  ...
  card:{width:"min(560px,92%)",padding:"42px 44px 38px",border:"1px solid #cbc7be",borderRadius:"14px",background:"rgba(250,249,246,.96)",boxShadow:"0 22px 70px rgba(37,41,37,.13)",textAlign:"center"},
  ...
  form:{display:"grid",gridTemplateColumns:"1fr 100px",gap:"10px"},
  input:{minWidth:0,height:"46px",padding:"0 14px",border:"1px solid #bdb8ad",borderRadius:"7px",background:"#fff",outline:"none",fontSize:"16px",letterSpacing:".04em",textAlign:"left",color:"#252925"},
  primary:{minHeight:"46px",display:"inline-flex",alignItems:"center",justifyContent:"center",gap:"7px",padding:"0 18px",border:0,borderRadius:"7px",background:"#3d554d",color:"#fff",fontSize:"13px",cursor:"pointer"},
