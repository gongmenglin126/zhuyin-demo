...
def between(text, start, end, replacement, label):
    ...
    if i<0 or j<0:
        raise SystemExit(f"missing source range: {label}")
...
# Admin: '诵录' is visible immediately and only burns itself.
p=Path('app/AdminPortalOccult.tsx')
s=p.read_text()
liturgy=r'''let liturgyBurned=false;
function Liturgy(){
...
'''
s=between(s,'const thresholdNamePost:Post={','const ritualFragmentPost:Post={',public,'public child post')
p.write_text(s)
...
- 管理员后台内部提供“返回论坛”入口；首次成功登录后只记住账号 `旧档员-03`，**不保存密码**。再次进入时玩家只需自行输入 `gumen-0712`；若忘记，可重新走旧档兼容认证小游戏取得口令。
'''
s=once(s,old,new,'canon horror rules')
p.write_text(s)
